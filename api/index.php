...
$actionList = ['admin','postLogin','upload','getImage','get_show_pic','postSignup','collect_info','getPersonalInfo'];
$noTokenList = ['admin','postLogin','upload','getImage','get_show_pic','postSignup','collect_info'];
...
if (in_array($_GET['_action'], $noTokenList)){//如果是不需要token的 action 直接进入
    ...
    // require_once "actions/{$_GET['_action']}.php";
    if (!isset($_SESSION['token'])){//无token错误
        Result::error('no token');
    }
    if(Token::userid($_SESSION['token']) < 1){//token不存在终止
        Result::error('token wrong');
    }
    //其余为正确情况 全局并进入action
    $GLOBALS['uid'] = Token::userid($_GET['token']);
